// AWS 콘솔에서 사용할 datalab-orchestrator 함수 코드 (ES Modules) - 수정된 버전
import { LambdaClient, InvokeCommand } from '@aws-sdk/client-lambda'

const lambdaClient = new LambdaClient({ region: 'ap-northeast-2' })

// 환경 변수에서 datalab-fetcher 함수명 가져오기 (기본값: 'datalab-fetcher')
const DATALAB_FETCHER_FUNCTION_NAME = process.env.DATALAB_FETCHER_FUNCTION_NAME || 'datalab-fetcher'
...
export const handler = async (event, context) => {
  ...
  try {
    ...
    console.log(`🔧 사용할 fetcher 함수: ${DATALAB_FETCHER_FUNCTION_NAME}`)
    ...
    const response = {
      ...
      data: allKeywords, // 크롤러 서비스에서 기대하는 필드명
      totalPages: maxPages,
...
async function invokeLambda(page, request) {
  ...
  try {
    ...
    const payload = {
      page,
      categoryId: request.categoryId, // 메인 앱과 동일하게 categoryId만 사용
    ...
    const command = new InvokeCommand({
      FunctionName: DATALAB_FETCHER_FUNCTION_NAME, // 환경변수에서 가져온 함수명 사용
    ...
    if (response.Payload) {
      ...
      console.log(`📊 수집된 키워드 수:`, result.keywords ? result.keywords.length : 0) // data → keywords

      return {
        page,
        success: result.success,
        data: result.keywords || result.data || [], // keywords 우선, data fallback
        error: result.error,
        responseTime,
      }
    } else {
