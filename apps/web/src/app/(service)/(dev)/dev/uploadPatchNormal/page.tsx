...
import { uploadPatchNormal } from './serverAction'
import { toast } from 'sonner'

export default function UploadPatchPage() {
  const { execute, isLoading, error, data } = useServerAction(uploadPatchNormal, {
    onSuccess: ({ message }) => {
  ...
  async function handleUpload() {
    if (!file) return
    const { public_url, s3Key } = await uploadS3PreSigned(file, ['patch_normal'])
