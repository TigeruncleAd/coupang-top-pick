...
export class NaverBestKeywordParser {
     ...
     */
    async parseHTML(htmlContent, url) {
     ...
     * 키워드 데이터 추출
     */
    async extractKeywords(root, url) {
        const keywords = [];
     ...
     */
    extractKeywordsFromCompressedHTML(root) {
        const keywords = [];
        try {
     ...
     */
    extractKeywordsFromRealHTML(root) {
        const keywords = [];
        try {
     ...
     */
    extractKeywordsFromNextJS(htmlContent) {
        const keywords = [];
        try {
            // Next.js 데이터 패턴 찾기
     ...
     * 텍스트 패턴에서 키워드 추출
     */
    extractKeywordsFromText(htmlContent) {
        const keywords = [];
        try {
            // 다양한 키워드 패턴 시도
            const patterns = [
     ...
     * 키워드 배열 파싱
     */
    parseKeywordArray(keywordArray) {
        const keywords = [];
     ...
     * 텍스트에서 키워드 파싱
     */
    parseKeywordFromText(text, rank) {
        if (!this.isValidKeyword(text))
            return null;
        // 순위, 키워드명, 변화 추출
        const patterns = [
     ...
     * 매치 결과에서 키워드 파싱
     */
    parseKeywordFromMatch(match, pattern) {
     ...
     */
    async extractKeywordsAlternative(root) {
        const elements = [];
        // 실제 HTML 구조에서 키워드와 순위 정보를 함께 찾기
        const keywordTitles = root.querySelectorAll('.rankingTitleResponsive_title__z9c4V, strong[class*="rankingTitleResponsive_title"], span[class*="rankingTitleResponsive_title"]');
        keywordTitles.forEach((titleElement, index) => {
            ...
            if (keyword && keyword.length > 1) {
                ...
                // 트렌드 정보 찾기
                let trend = '유지';
                parent = titleElement.parentElement;
                while (parent && parent !== root) {
                    const parentText = parent.textContent || '';
     ...
     * 가상 키워드 요소 생성
     */
    createVirtualKeywordElement(rank, trend, keyword) {
     ...
     * 키워드 요소 파싱
     */
    parseKeywordElement(element, defaultRank, url) {
        try {
            // 순위 추출
     ...
     * 순위 추출
     */
    extractRank(element, defaultRank) {
        // 순위 선택자들
     ...
     * 관련 상품 추출
     */
    extractRelatedProducts(element) {
        const products = [];
        // 관련 상품 선택자들
