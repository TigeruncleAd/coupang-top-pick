...
import { User, Truck, CreditCard, ListChecks, ChevronRight, Check } from 'lucide-react'
      ...
      <div className="bg-muted flex items-center gap-1 rounded-lg p-0.5">
        {steps.map((step, index) => (
          <div key={index} className="flex items-center gap-1">
            ...
            {index < steps.length - 1 && <ChevronRight className="text-muted-foreground h-4 w-4" />}
          </div>
        ))}
      </div>
    </div>
...
export function Button9() {
  ...
  return <Steps steps={steps} activeStep={activeStep} onStepChange={setActiveStep} />
}
