  ...
  const handleToggle = (integrationName: string) => {
    setActiveIntegrations(prevIntegrations =>
      prevIntegrations.map(integration =>
        integration.name === integrationName ? { ...integration, enabled: !integration.enabled } : integration,
      ),
    )
  }
              ...
              {activeIntegrations.map(integration => (
                <Card key={integration.name} className="border shadow-sm">
                  ...
                  <div className="flex items-center justify-between px-4 py-4 md:px-6">
                    ...
                    <Switch checked={integration.enabled} onCheckedChange={() => handleToggle(integration.name)} />
                  </div>
                </Card>
              ))}
            </div>
